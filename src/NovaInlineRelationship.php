...
class NovaInlineRelationshipExtended extends Field
{
    ...
    protected function setMetaFromClass(array $item, $attrib, $value = null, $resource = null)
    {
        ...
        $item['meta']['placeholder'] = $item['placeholder'] ?? __('Add').' '.$item['meta']['singularLabel'];
    ...
    protected function getPropertiesFromFields(Collection $fields): Collection
    {
        ...
        if ($request->isCreateOrAttachRequest()) {
            $customRules = 'creationRules';
        } else if ($request->isUpdateOrUpdateAttachedRequest()) {
            $customRules = 'updateRules';
        } else {
            $customRules = null;
        }
        return $fields->map(function ($value) use($customRules) {
            return [
